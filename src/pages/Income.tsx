...
const categories = ['Certificate Fee','Inspection Fee','Laboratory Fee','Vaccination Fee','Other Income'];
...
export default function Income() {
  ...
  const submit = () => {
    ...
    addIncome({ id: Date.now(), category: form.category, amount: Number(form.amount), description: form.description, date: new Date().toISOString().slice(0,10), account: form.account });
            ...
            <tbody>
              {incomes.map(i => (
                <tr key={i.id} className="border-b border-slate-100 last:border-0">
                  <td className="p-3">{i.category}</td><td className="p-3">{i.description}</td><td className="p-3">{i.account}</td>
                  <td className="p-3 font-bold text-emerald-600">+${i.amount}</td><td className="p-3 text-slate-500">{i.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <div className="mt-3 text-sm font-bold">Total Income: <span className="text-emerald-600">${total.toLocaleString()}</span></div>
      </div>
      <div className="bg-white border border-slate-200 rounded-xl p-5 h-fit">
        <h3 className="text-sm font-bold mb-3.5">Add Income</h3>
        <div className="flex flex-col gap-3">
          ...
          <button onClick={submit} className="bg-brand-600 text-white px-4 py-2.5 rounded-lg text-sm font-bold">Add Income</button>
        </div>
      </div>
    </div>
  );
